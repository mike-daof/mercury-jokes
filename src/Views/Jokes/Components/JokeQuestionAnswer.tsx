...
export default function JokeQuestionAnswer({ joke, showAnswer, onClick }: JokeQuestionAnswerProps) {
  return (
    <Stack direction='column' alignItems='center'>
      ...
      <Button onClick={onClick} className='h-16 rounded-full capitalize' variant="contained" size="medium"><Typography className="capitalize">{showAnswer ? 'Hide punchline' : 'Show punchline'}</Typography></Button>
      {showAnswer && (
